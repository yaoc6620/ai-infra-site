...
### 5.3 为什么一开始选择 Per-layer Stream（以及为什么这是过度防御）

发现 NaN 后，定位到是"全局 stream + CUDA Graph"导致的内存问题。当时有**两个怀疑方向**：

| 怀疑 | 场景 | 实际安全？ |
|------|------|-----------|
| 跨层同 stream 复用 | Layer 0 在 dense_stream 上 free → Layer 1 在**同一个** dense_stream 上 alloc 复用 | ✅ **安全**（同 stream 顺序保证） |
| 跨 stream 复用 | default_stream 上 free → dense_stream 上还在读 | ❌ **不安全**（无顺序保证） |

当时**分不清是哪个导致的 NaN**，所以采取了双保险策略——一次做两件事：

```python
# 1. Per-layer stream → 防止跨层复用（其实不需要）
self._dense_stream = torch.cuda.Stream()
self._moe_stream = torch.cuda.Stream()

# 2. record_stream() → 防止跨 stream 复用（这才是真正的 fix）
residual.record_stream(dense_stream)
```

**事后分析**：

"跨层同 stream 复用"其实是安全的——同一个 stream 上 Layer 0 的 kernel 一定在 Layer 1 之前执行完：
```
shared_dense_stream:
  Layer 0 [alloc A] → [用 A] → [free A] → Layer 1 [alloc B 复用 A 的地址]
                                                     ↑ 此时 Layer 0 早就不读 A 了，安全
```

真正的 NaN 只来自"跨 stream 复用"：
```
default_stream:  [创建 residual] → ... → [引用归零 free] → [后续 alloc 复用地址，写入新数据]
dense_stream:    [...还在读 residual 这个地址...] → 读到脏数据 → NaN
```

**结论**：per-layer stream 是排查时的过度防御。真正修 NaN 的只有 `record_stream()`。但这种"先加最严格隔离修好 bug，再逐步放松找到最小必要修复"的 debug 方法论是合理的。
