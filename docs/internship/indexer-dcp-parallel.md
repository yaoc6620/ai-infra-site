...
3. **DCP=8 chunk=4096 OOM**：compact buffer = `ceil(128000/1024) × 1024 × 128 × 2B ≈ 32MB`，每个 non-owner 都需要这块内存，7 个 non-owner 共 ~224MB 额外占用
4. **DCP=16 支持更大 chunk**：每 rank 分到的 KV 更少，cache_block_size = 128×16 = 2048，compact buffer 更小
...
A: compact buffer = `ceil(Skv/cache_block_size) × 1024 × 128 × 2B`。128k prompt 时约 32MB/rank。7 个 non-owner 的 `_broadcast_cache_buf` 共 ~224MB，加上 owner 的临时拷贝，总额外显存超过可用裕量。DCP=16 时 cache_block_size=2048，compact buffer 更小，所以能跑 chunk=4096 甚至 8192。
