...
## 二、Profiling 分析：怎么发现这里值得优化

### 2.1 从 Timeline 上发现问题

用 Nsight Systems / Ascend Profiler 抓 decode 阶段单步的 timeline，观察单层 MLA Attention 的 kernel 排布：

```
decode 阶段 timeline（单层 MLA，原始串行）:

主 Stream:
  [q_a_proj 0.1ms] → [layernorm 0.05ms] → [indexer ██████ 2.5ms] → [q_b_proj 0.3ms] → [RoPE 0.2ms] → [sparse_attn 1.0ms]
...
**关键观察**：
1. indexer 占了 ~2.5ms，是这段流水线中**耗时最长的单个环节**
2. indexer 之后的 q_b_proj + RoPE 加起来 ~0.5ms，**它们不依赖 indexer 的输出**（只依赖 q_c，而 q_c 在 indexer 之前就算好了）
3. indexer 的结果（topk_indices）直到最后的 sparse_attn 才被消费

→ 这 0.5ms 的小计算完全可以和 indexer 并行，被 "藏" 在 indexer 的执行时间里。

### 2.2 判断 overlap 收益

**overlap 节省的时间 = 被重叠掉的那段计算的耗时**：

```
优化前: indexer(2.5ms) → q_b_proj+RoPE(0.5ms) → sparse_attn  总 = 4.1ms
优化后: indexer(2.5ms)  ← overlap →  q_b_proj+RoPE(0.5ms)
                                      ↑ 被 indexer 覆盖，"免费"执行
        → sparse_attn                                         总 = 3.6ms
                                                               节省 ~0.5ms/layer
```

60 层累积 → 每步节省约 **2ms**（实测值，每层不完全一样）。

### 2.3 为什么 Prefill 阶段不做

Prefill 阶段和 Decode 的 Profiling 图完全不同：

```
prefill 阶段 timeline（单层 MLA，batch=64K tokens）:

主 Stream:
  [q_a_proj ████████ 8ms] → [layernorm 2ms] → [indexer ██ 3ms] → [q_b_proj ████████ 8ms] → [sparse_attn ████████████ 15ms]
```

**差异**：
- Prefill 的 q_b_proj 从 0.3ms 变成 ~8ms（64K tokens 的大矩阵乘法），**SM 利用率已经很高**
- 两个 stream 的 kernel 会**争抢 SM 资源**——GPU 并不是两个 stream 各跑各的，而是所有 kernel 共享同一组 SM。当 SM 已经被大 kernel 打满时，side stream 的 kernel 抢不到 SM，反而互相拖慢
- indexer（3ms）相对总耗时（36ms+）占比极小，overlap 能省的那点时间可以忽略
- 反而引入了多 stream 同步开销和更复杂的内存管理

**结论**：Decode 阶段是 **launch-bound / memory-bound**（小 tensor，SM 空闲），overlap 有效。Prefill 阶段是 **compute-bound**（大矩阵，SM 满载），overlap 无收益甚至有害。
...
### 5.2 生产者侧：模型层（gpu_mt_flash_moe.py）
...
### 5.3 消费者侧：Attention 层（flash_attn_v3_sparse.py）
